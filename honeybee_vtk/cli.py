...
@click.option(
    '--file-type', '-ft', type=click.Choice(['html', 'vtkjs']), default='html',
    help='Switch between html and vtkjs formats', show_default=True
)
@click.option(
    '--display-mode', '-dm', type=click.Choice(['shaded', 'surface',
        'surfacewithedges', 'wireframe', 'points']), help='Set display mode for the'
        ' model.'
)
@click.option(
    '--grid-options', '-go', type=click.Choice(['ignore', 'points', 'meshes']),
    help='Export sensor grids as either points or meshes.'
)
@click.option(
